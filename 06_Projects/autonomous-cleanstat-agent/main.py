...
def get_workspace_state(workspace_dir: str) -> Dict[str, Any]:
    ...
    if workspace.exists():
        for item in workspace.rglob("*"):
            if item.is_file():
                state["files"].append(str(item.relative_to(workspace)))
    
    # Count pending observations
    queue_dir = workspace / "data" / "queue"
    if queue_dir.exists():
        state["observations_pending"] = len(list(queue_dir.glob("*.json")))
    
    return state


def run_autonomous_loop(task: Dict[str, Any], governor: Governor, agent: Agent, executor: Executor, workspace_dir: str):
    ...
    state = {
        ...
        "last_action": "",
        "last_result": {},
        "workspace": get_workspace_state(workspace_dir)
    }
...
def main():
    ...
    governor = Governor()
    agent = Agent(ollama_base_url=ollama_base_url, model=ollama_model)
    executor = Executor(cleanstat_api_url=cleanstat_api_url, cleanstat_api_key=cleanstat_api_key, workspace_dir=workspace_dir)
    ...
    if not Path(task_file).exists():
        ...
        Path(f"{workspace_dir}/data/images").mkdir(parents=True, exist_ok=True)
        
        # Create a dummy image for testing
        from PIL import Image
        img = Image.new('RGB', (100, 100), color='red')
        img.save(f"{workspace_dir}/{sample_task['image_path']}")
    ...
    # Run autonomous loop
    run_autonomous_loop(task, governor, agent, executor, workspace_dir)
