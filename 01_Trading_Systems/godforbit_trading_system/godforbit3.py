...
CAPITAL_ACTIVE_RATIO   = 0.60   # 60% available for trading
CAPITAL_RESERVE_RATIO  = 0.20   # 20% untouchable reserve
CAPITAL_BUFFER_RATIO   = 0.20   # 20% risk buffer (used only in drawdown)
...
SPREAD_TARGET_PCT      = 0.003    # 0.3% target per leg
...
class CopyThatPay:
    ...
    async def flatten_symbol(self, symbol: str, reason: str = '') -> None:
        ...
        self.journal.log('FLATTEN', symbol, '', 0, reason=reason,
                         hedge_state='flat', risk_snapshot=snap)
    ...
    async def place_spread_exit(self, symbol: str, side: str, cts: float,
        ...
        try:
            ...
            console.print(
                ...
                f'(entry={entry:.8f} profit=${profit_est:.6f})')
            return order.get('id', '')
        except Exception as e:
    ...
    async def _manage_side(self, symbol: str, side: str, qs: QuoteState) -> None:
        ...
        if filled and entry > 0 and cts > 0:
            if not exit_id:
                eid = await self.place_spread_exit(symbol, side, cts, entry)
                if is_long:
                    qs.long_exit_id = eid or ''
                else:
                    qs.short_exit_id = eid or ''
            else:
