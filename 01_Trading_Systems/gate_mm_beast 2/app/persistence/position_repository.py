...
class PositionRepository:
    ...
    def update_open_position(self, symbol: str, side: str, qty: int, entry_vwap: float, payload: dict | None = None) -> None:
        with self.db.conn() as c:
            c.execute(
                "UPDATE positions SET side=?, qty=?, entry_vwap=?, payload_json=? WHERE symbol=? AND status='open'",
                (side, qty, entry_vwap, json.dumps(payload or {}), symbol),
            )
            c.commit()
